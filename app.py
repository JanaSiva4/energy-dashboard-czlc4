...
def generovat_pdf_protokol(zamestnanec, sklad, kvartal, vydane_polozky, vedouci):
    ...
    ph_s    = ParagraphStyle('ph',  fontSize=9,  fontName=PDF_FONT_BOLD, textColor=colors.white)
    ...
    ht = Table([
        [Paragraph("PREDAVACI PROTOKOL - MCDP", title_s)],
        [Paragraph(f"Myci a cistici prostredky - Sklad {sklad} - Chrastany - Facility", sub_s)]
    ], colWidths=[17*cm])
    ...
    it = Table([
        [Paragraph('Zamestnanec:', label_s), Paragraph(f'<b>{zamestnanec}</b>', body_s),
         Paragraph('Kvartal / Rok:', label_s), Paragraph(f'<b>{kvartal}</b>', body_s)],
        [Paragraph('Sklad:', label_s), Paragraph(sklad, body_s),
         Paragraph('Datum vydeje:', label_s), Paragraph(datetime.now().strftime('%d.%m.%Y'), body_s)],
        [Paragraph('Vedouci:', label_s), Paragraph(vedouci or '-', body_s),
    ...
    polozky_data = [
        [Paragraph('Polozka', ph_s), Paragraph('Vydano', ph_s),
         Paragraph('Specifikace', ph_s), Paragraph('Prevzal (podpis)', ph_s)],
        [Paragraph('1x Rucnik 50x100cm Siguro', body_s),
         'V' if vydane_polozky.get('rucnik') else 'O', '50x100 cm, frote', ''],
        [Paragraph('1x Tekute mydlo', body_s),
         'V' if vydane_polozky.get('mydlo') else 'O', '500 ml', ''],
        [Paragraph('1x Ariel tablety 60 ks', body_s),
         'V' if vydane_polozky.get('ariel') else 'O', '60 ks / baleni', ''],
        [Paragraph('1x Krem Indulona original', body_s),
         'V' if vydane_polozky.get('krem') else 'O', 'nebo mesickovy', ''],
        [Paragraph('1x Abrazivni pasta Solvina', body_s),
         'V' if vydane_polozky.get('solvina') else 'O', '450 g', ''],
    ]
    ...
    st_tbl = Table([
        [Paragraph('Podpis zamestnance:', label_s), '',
         Paragraph('Podpis vedouciho / razitko:', label_s), ''],
        ['', '', '', ''],
        ['', '', '', ''],
    ...
    lht = Table([[Paragraph('Prohlaseni zamestnance - NV 390/2021 Sb.',
        ParagraphStyle('lh', fontSize=9, fontName=PDF_FONT_BOLD, textColor=colors.white))]],
        colWidths=[17*cm])
    lht.setStyle(TableStyle([
    ...
    legal_txt = (
        "Predani a prevzeti vyse uvedenych OOPP a predmetu zamestnanec i zamestnavatel "
        "potvrzuji svym podpisem. Dale byl zamestnanec seznamen se zpusobem udrzby OOPP "
        "dle NV 390/2021 Sb. Zamestnanec se zavazuje radne hospodarit s OOPP a predmety "
        "sverenymi mu zamestnavatelem na zaklade tohoto potvrzeni a strezit a ochranovat "
        "tyto OOPP a predmety zamestnavatele pred poskozenim, ztratou, znicenim a zneuzitim. "
        "Zamestnanec se zavazuje sverene OOPP pouzivat pouze pro vykon prace pro zamestnavatele. "
        "Zamestnanec souhlasi s tim, ze v pripade ztraty nebo poskozeni bude cena srazena "
        "ze mzdy v souladu s prislusnou Dohodou o srazkach ze mzdy."
    )
...
# ── OOPP & MCDP ──────────────────────────────────────────────────
    ...
    with col_main:
        ...
        if rezim == "Výdej MČDP":
            ...
            if zamestnanec2 and email_zam2:
                ...
                if st.button("✅ ULOŽIT DO EVIDENCE", use_container_width=False):
                    if not zamestnanec2:
                        ...
                        vybrane = [(nazev, klic, exp_mes) for nazev, klic, exp_mes in pomucky_def if vydane.get(klic)]
                        if not vybrane:
                            ...
                            st.error("❌ Nepodařilo se uložit. Zkontroluj připojení.")
            else:
            ...
            # OPRAVA: PDF se generuje vzdy, download button je vzdy pritomen
