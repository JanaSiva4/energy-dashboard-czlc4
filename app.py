...
<style>
    ...
    div[data-testid="stMetric"] {
        ...
        padding: 20px;
        border-radius: 15px;
        border: 1px solid #00aaff; /* Zjemněný okraj */
    }
    ...
    /* Styl pro Archivní sekce - jemnější vzhled */
    .archive-card {
        background-color: rgba(255, 255, 255, 0.03);
        padding: 12px;
        border-radius: 8px;
        border-left: 3px solid #00aaff; /* Tenčí linka */
        margin-bottom: 8px;
        font-size: 0.9em;
        color: #e0e0e0;
    }
...
with col_main:
    ...
    with tab2:
        ...
        # Jemnější karty bez tučného písma
        st.markdown("""
        <div class="archive-card">Účtenka - Pohonné hmoty (Shell) - 1.250 Kč <span style='color:#666; font-size: 0.8em; margin-left: 10px;'>[Simulace]</span></div>
        <div class="archive-card">Jízdenka - ČD (Praha-Brno) - 340 Kč <span style='color:#666; font-size: 0.8em; margin-left: 10px;'>[Simulace]</span></div>
        """, unsafe_allow_html=True)

    with tab3:
        st.write("### Ostatní provozní faktury")
        st.markdown("""
        <div class="archive-card">Kancelářské potřeby - 4.200 Kč <span style='color:#666; font-size: 0.8em; margin-left: 10px;'>[Simulace]</span></div>
        <div class="archive-card">Čistící prostředky - 1.150 Kč <span style='color:#666; font-size: 0.8em; margin-left: 10px;'>[Simulace]</span></div>
        """, unsafe_allow_html=True)
