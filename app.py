...
# Inicializace stavu (aby data zůstala i po interakci)
...
with c2: st.metric("Celková suma", "Dle dat")
    ...
    if st.session_state.vysledky:
        ...
        if len(vyber) > 0:
            ...
            fig = px.bar(df, x="Faktura", y=y_col, title=f"Srovnání: {y_col}", 
                         ...
                         color_discrete_sequence=['#808080']) # Tady je ta šedá
            fig.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
            st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("💡 Nahrajte faktury vlevo pro spuštění.")
