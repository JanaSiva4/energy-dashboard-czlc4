...
def generovat_pdf_protokol(zamestnanec, sklad, kvartal, vydane_polozky, vedouci):
    ...
    title_s  = ParagraphStyle('t', fontSize=15, fontName=PDF_FONT_BOLD, textColor=colors.white, alignment=1)
    sub_s    = ParagraphStyle('s', fontSize=9,  fontName=PDF_FONT, textColor=colors.white, alignment=1)
    label_s  = ParagraphStyle('l', fontSize=9,  fontName=PDF_FONT_BOLD, textColor=navy)
    body_s   = ParagraphStyle('b', fontSize=9,  fontName=PDF_FONT, textColor=colors.HexColor('#333333'))
    legal_s  = ParagraphStyle('leg', fontSize=7.5, fontName=PDF_FONT, textColor=colors.HexColor('#444444'), leading=10)
    ...
    ph = ParagraphStyle('ph', fontSize=9, fontName=PDF_FONT_BOLD, textColor=colors.white)
    ...
    lht = Table([[Paragraph('Prohlášení zaměstnance — NV 390/2021 Sb.',
        ParagraphStyle('lh', fontSize=9, fontName=PDF_FONT_BOLD, textColor=colors.white))]],
        colWidths=[17*cm])
...
for col, (icon, name, desc) in zip(cols_kat, kategorie_list):
    with col:
        ...
        st.markdown(f"""<div class="cat-card {active}"><div style="font-size:1.8rem">{icon}</div><div class="cat-name">{name}</div><div class="cat-desc">{desc}</div></div>""", unsafe_allow_html=True)
    ...
    with col_main:
        ...
        if st.session_state.vysledky:
            ...
            with col_e2:
                ...
                st.download_button("⬇ Export Excel", data=buffer.getvalue(), file_name=f"DocScan_{periode}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
# ── OOPP & MČDP ──────────────────────────────────────────────────
    ...
    with col_main:
        ...
        if rezim == "Výdej MČDP":
            ...
            kvartal_sel = st.selectbox("Kvartál", [f"Q1 / {rok_akt}", f"Q2 / {rok_akt}", f"Q3 / {rok_akt}", f"Q4 / {rok_akt}", f"Q1 / {rok_akt+1}", f"Q2 / {rok_akt+1}", f"Q3 / {rok_akt+1}", f"Q4 / {rok_akt+1}"])
            ...
            if zamestnanec and email_zam:
                ...
                qr_data = {"jmeno": zamestnanec, "email": email_zam, "stredisko": stredisko, "user": user, "sklad": sklad_oopp, "kvartal": kvartal_sel, "polozky": ", ".join(polozky_list)}
                ...
                with col_info:
                    st.markdown(f'''<div class="energy-card oopp-border" style="padding:12px;">
                      <p style="color:#00c864;font-size:0.75rem;font-weight:bold;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">Čeká na 2FA podpis</p>
                      <p style="color:#fff;font-size:0.9rem;"><b>{zamestnanec}</b></p>
                      <p style="color:#aaa;font-size:0.8rem;">{email_zam}</p>
                      <p style="color:#aaa;font-size:0.8rem;margin-top:4px;">{kvartal_sel} · {sklad_oopp}</p>
                      <p style="color:#aaa;font-size:0.75rem;margin-top:4px;">{", ".join(polozky_list)}</p>
            ...
            with col_btn2:
                if zamestnanec:
                    pdf_bytes = generovat_pdf_protokol(zamestnanec=zamestnanec, sklad=sklad_oopp, kvartal=kvartal_sel, vydane_polozky={"rucnik": rucnik, "mydlo": mydlo, "ariel": ariel, "krem": krem, "solvina": solvina}, vedouci=vedouci)
                    jmeno_souboru = zamestnanec.replace(" ", "_")
                    st.download_button(label="📄 Stáhnout PDF protokol", data=bytes(pdf_bytes), file_name=f"Protokol_MCDP_{jmeno_souboru}_{kvartal_sel[:2]}.pdf", mime="application/pdf", use_container_width=True, key=f"dl_mcdp_{st.session_state.mcdp_reset}")
            ...
            # QR kód — pouze když je jméno a email
            if zamestnanec2 and email_zam2:
                vydane_nazvy = [nazev for nazev, klic, _ in pomucky_def if vydane.get(klic)]
                oopp_qr_data = {"jmeno": zamestnanec2, "email": email_zam2, "stredisko": stredisko2, "user": user2, "sklad": sklad_oopp, "kvartal": f"OOPP {datetime.now().strftime('%m/%Y')}", "polozky": ", ".join(vydane_nazvy) if vydane_nazvy else "—"}
                ...
                with col_info2:
                    ...
                    st.markdown(f'''<div class="energy-card oopp-border" style="padding:12px;">
                      ...
                      <p style="color:#aaa;font-size:0.75rem;margin-top:4px;">{nazvy_str}</p>
                    </div>''', unsafe_allow_html=True)
                st.write("---")

            # OPRAVA: Tlačítko ULOŽIT je vždy viditelné — stejně jako u MČDP
            st.write("---")
            if st.button("✅ ULOŽIT DO EVIDENCE", use_container_width=False, key="btn_ulozit_oopp"):
                if not zamestnanec2:
                    st.warning("⚠️ Zadej jméno zaměstnance.")
                else:
                    vybrane = [(n, k, e) for n, k, e in pomucky_def if vydane.get(k)]
                    if not vybrane:
                        st.warning("⚠️ Nezaškrtla jsi žádnou pomůcku.")
                    else:
                        ulozeno = 0
                        chyby = []
                        for nazev, klic, exp_mes in vybrane:
                        ...
                        if ulozeno > 0:
                            ...
                            if chyby:
                                st.warning(f"⚠️ Nepodařilo se uložit: {', '.join(chyby)}")
                            st.session_state.oopp_reset += 1
                            st.rerun()
                        else:
                            st.error("❌ Nepodařilo se uložit. Zkontroluj připojení k Sheets.")
            ...
            kv_tisk  = st.selectbox("Kvartál", [f"Q1 / {rok_akt2}", f"Q2 / {rok_akt2}", f"Q3 / {rok_akt2}", f"Q4 / {rok_akt2}", f"Q1 / {rok_akt2+1}", f"Q2 / {rok_akt2+1}", f"Q3 / {rok_akt2+1}", f"Q4 / {rok_akt2+1}"], key="kv_tisk")
